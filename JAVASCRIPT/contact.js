...
function sendEmail() {
    ...
    const bodyMessage = ` FullName: ${name1.value}<br> Email:${email.value}<br> Phone Number:${completePhone}<br> message:${message.value}`
...
function validateName() {
    ...
    const userName = document.querySelector(".error-txt name"); // Replace this with the name you want to validat


    if (!name1.value.match(nameRegex)) {
        name1.classList.add("error");
        name1.parentElement.classList.add("error");

        if (name1.value != "") {
            userName.innerText = "Enter a Valid name";
            return false;
        }
        else {
            userName.innerText = "Type Correct name "
            return false;
        }
        return false;
        ...
        name1.parentElement.classList.remove("error");
        name1.parentElement.classList.remove("error");
        return true;
...
function checkPhone() {
    ...
    if (!phone.value.match(mobileNumberRegex)) {
        ...
        phone.parentElement.classList.remove("error");
        completePhone = document.getElementById("countryCode").innerText + " " + phone.value;
        return true;
...
for (country of countries) {
    ...
    <li class="option">
        ...
        <strong id='countryCode'>+${country.phone}</strong>
    </li> `;
...
form.addEventListener("submit", (e) => {
    ...
    if (!name1.classList.contains("error")&& !email.classList.contains("error")
